...
public class GameBoard {
     ...
     * algorithm.
     */
    private void shuffle(Object array[], Random r) { // IGNORE THIS METHOD.***
     ...
     * returned.
     */
    public String isWord(String word) { //must call 'findPath' method
		for (int i = 0; i < NUM_ROWS; i++) {
			...
			for (int j = 0; j < NUM_COLS; j++) {
				...
				if (word.charAt(0) == board[i][j]) {
					...
					Stack<String> pathSoFar = new Stack<String>();
					visited = new boolean[NUM_ROWS][NUM_COLS];
					pathSoFar.push("(" + i + ", " + j + ")");
					visited[i][j] = true;
					if (findPath(word.substring(1, word.length()), pathSoFar, i, j)) {
    ...
    } // End isWord. DOES NOT PRINT OUT SEQUENCE.
    ...
    private boolean findPath(String wordToFind, Stack<String> pathSoFar, int row, int col) {
		...
		if (wordToFind.length() < 1 ) { // 'wordToFind' is of length 0 and it is found)
			return true;
		} // end if

		try {

			if (wordToFind.charAt(0) == board[row - 1][col]) {
				// next letter is found in look left(row - 1)

				pathSoFar.push("(" + (row-1) + ", " + col + ")"); // push coordinate to stack 'pathSoFar'
				visited[row-1][col] = true;
				if (findPath(wordToFind.substring(1, wordToFind.length()), pathSoFar, row-1, col)) {
					return true;
				} // end if
		...
		} catch (IndexOutOfBoundsException e){} // end try/catch
		...
		try {
			...
			if (wordToFind.charAt(0) == board[row][col-1]) {
				//next letter is found in look down(col - 1)

				pathSoFar.push("(" + row + ", " + (col-1) + ")"); // push coordinate to stack 'pathSoFar'
				visited[row][col-1] = true;
				if (findPath(wordToFind.substring(1, wordToFind.length()), pathSoFar, row, col-1)) {
					return true;
				} // end if
		...
		try {
			...
			if (wordToFind.charAt(0) == board[row][col+1]) {
				//next letter is found in look up(col + 1)
		...
		try {
			...
			if (wordToFind.charAt(0) == board[row+1][col-1]) {
				//next letter is found in look rightDown(row+1,col-1)

				pathSoFar.push("(" + (row+1) + ", " + (col-1) + ")"); // push coordinate to stack 'pathSoFar'
				visited[row+1][col-1] = true;
				if (findPath(wordToFind.substring(1, wordToFind.length()), pathSoFar, row+1, col-1)) {
					return true;
				} // end if
		...
		try {
			...
			if (wordToFind.charAt(0) == board[row+1][col+1]) {
				//next letter is found in look rightUp(row+1,col+1)
		...
		try {
			...
			if (wordToFind.charAt(0) == board[row-1][col+1]) {
				//next letter is found in look leftUp(row-1,col+1)
		...
		try {
			...
			if (wordToFind.charAt(0) == board[row-1][col-1]) {
				//next letter is found in look leftDown(row-1,col-1)
...
/* Given cell (i,j), are we on a B? (Base case); BACKTRACK--> Target word: 'BATH'
if no --> return false;

else (recursion) solve problem by moving 8, asking if we can form 'th'
Look at review notes from lab 6
*/
